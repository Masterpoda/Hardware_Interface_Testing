...
#define ADDRESS 0x02        //the I2C and RF address of the antenna controller
...
float movementSpeed =  0.05; //Maximum estimated movement speed in m/s (Safest to over-estimate) Set to 0 to calibrate delay
      ...
      digitalWrite(4, HIGH);
        ...
        if(!networkMsgs.eqDWMsg())
        {
          protocolFailed = true;  //messages are out of sync, wait until next poll
            ...
            Serial.print("Expected: ");printDWMessage(expectedMessage);
            Serial.print("Recieved: ");printDWMessage(recievedMessage);
          #endif
        ...
        if (networkMsgs.recType() == POLL) {
          ...
          {
            if(networkMsgs.recFrom() == 0x03)               //message
            {
        ...
        else if (networkMsgs.recType() == RANGE && !protocolFailed) //if we recieved a range message and we aren't out of sync
        {
          ...
          {   
              ...
              //Serial.print("Sending Range Report to ");Serial.println(recievedMessage.from_address);
        ...
        else if(networkMsgs.recType() == POLL_ACK && !protocolFailed)
        {
          networkMsgs.setToNextExpected();  //poll ack is to other anchor
        }
        else if(networkMsgs.recType() == RANGE_REPORT && !protocolFailed) //other anchor is sending range report
        {
          networkMsgs.setToNextExpected();  //range report is from other anchor
        }
