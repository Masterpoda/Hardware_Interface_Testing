...
#define ADDRESS 0x04      //Tag 3, placed on right of hopper, Tag 4 on the left
...
void resetInactive() {
    ...
    //delay(resetPeriod); //ensure all other antennas reset.
...
void loop() {
    ...
    if (sentAck) {
      digitalWrite(5, HIGH);
        ...
        if (msgId == POLL) {
            ...
            if(networkMsgs.getIndex() == 0)                       //sent a poll to Anchor 1 means we've reset
              protocolFailed = false;
    ...
    if (receivedAck) 
    {
      digitalWrite(4, HIGH);
        ...
        if(networkMsgs.recType() == POLL)//we're hearing the poll from Tag 3
        { 
          if(networkMsgs.recTo() == ANCHOR_1_ADDRESS)
            protocolFailed = false;
        ...
        else if (networkMsgs.recType() == RANGE_REPORT && !protocolFailed) //if we recieve a range report 
        {
          ...
          if(networkMsgs.recTo() == ADDRESS)//range report is meant for us, other tag will poll next
          {
            float curRange;
          ...
          else  //range report is meant for other tag, we will poll next
          {
            networkMsgs.setToNextExpected();         
            transmitPoll(networkMsgs.recFrom()); //Tag 4 polls anchor that just reportted to Tag 3
